...
class Delaunay {
    ...
    func compute () -> Array<Int>? {
        ...
        var circleList = CircleList(points: [0.0, 0.0, Float(maxTriangleRadius)])
...
for i in 0..<points {
    var x = Int(arc4random()) % width
    var y = Int(arc4random()) % height
...
var d = Delaunay(points: pointList)
var results = d.compute()!
